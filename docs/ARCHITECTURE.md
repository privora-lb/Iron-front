...
| `src/world/`    | `terrain.js` (the cell model), `hydrology.js` (where water runs), `roads.js` (where roads go), `vegetation.js` (what grows), `landuse.js` (the field patchwork) | Generation in an order, each step a consequence of the last. Plain data; no canvas, no engine.                                                                 |
| `src/render/`   | `three/` — the 3D battlefield: ground mesh, instanced props and units, camera                                                                                   | A second renderer, loaded on demand. It reads the world and never writes to it.                                                                                |
