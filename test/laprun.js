...
try {
  ...
  const stuck = g.hook("stuck"); const lap = g.hook("squadlap");
  ...
  for (const at of checks) {
    ...
    const s = stuck();
    const L = lap(); out.points.push({ tick: at, inSolid: s.inSolid, squads: L.squads, lapPairs: L.pairs, lapWorst: L.worstFrac });
  }
