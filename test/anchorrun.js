...
try {
  ...
  const stuck = g.hook("stuck"); const sq = g.hook("stuckSq");
  ...
  for (const at of checks) {
    ...
    const s = stuck();
    const q = sq(); out.points.push({ tick: at, menInSolid: s.inSolid, anchorsInSolid: q.anchorsInSolid, byKind: q.byKind });
  }
